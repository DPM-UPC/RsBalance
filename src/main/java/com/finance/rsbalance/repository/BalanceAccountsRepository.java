...
public interface BalanceAccountsRepository extends JpaRepository<BalanceAccounts, Long> {
    ...
    @Query("SELECT ba.id.monthly_period as period, ba.id.user_business_id as business, ba.id.account_id as account, amount as amount  FROM BalanceAccounts ba WHERE ba.id.monthly_period = :period and ba.id.user_business_id = :business and ba.id.account_id = :account")
